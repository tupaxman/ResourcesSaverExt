...
function resetListSites() {
    downloadSites.sites = downloadSites.sites.map(function(item) {
...
function updateListSites(idx, item) {
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.getElementById('edit-list').addEventListener('click', function () {
        ...
        document.getElementById('list-input-area').value = downloadSites.sites.map(function (site) {
            return site.url;
        }).join('\n');
    });
...
function tabCompleteForXHRHandler(tabId, changeInfo, keepDisabled) {
...
async function saveAllResourcesFromList(e) {
    // Disable button
    ...
    for (var site of downloadSites.sites) {
        ...
        try {
            await new Promise(function (resolve, reject) {
                ...
                var currentIdx = idx;
                var tabChangeHandler = function (tabId, changeInfo) {
                    if (tabId === chrome.devtools.inspectedWindow.tabId && changeInfo && changeInfo.status) {
                        ...
                        if (changeInfo.status === 'complete') {
                            ...
                            chrome.devtools.network.getHAR(result => {
                                var foundPage = result.pages.find(function(page) {
                                    return cleanUrlForComparison(page.title) === cleanUrlForComparison(currentSite.url);
                                });
                                if (foundPage) {
                                    setTimeout(function() {
                                        saveAllResources(e, function(isSuccess) {
                                            if (isSuccess) {
...
function saveAllResources(e, callback) {
    var toDownload = [];
            ...
            chrome.devtools.inspectedWindow.getResources(function (resources) {
                ...
                processContentFromResources(allResources, function (combineResources) {
                    ...
                    if (document.getElementById('check-zip').checked) {
                        ...
                        downloadZipFile(toDownload, function(isSuccess) {
                            allDone(isSuccess);
                            callback(isSuccess);
                        });
                    } else {
                        downloadListWithThread(toDownload, downloadThread, function(isSuccess) {
                            allDone(isSuccess);
                            callback(isSuccess);
                        });
                    }
...
//         code: 'window.addEventListener("load", function() { chrome.runtime.sendMessage({type: "RELOADED"}); }, false); window.onload = function() { chrome.runtime.sendMessage({type: "RELOADED"}); };'
//     });
// }, 3000);
